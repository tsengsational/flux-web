...
const { data: productions, error } = await useAsyncData(`production-${slug}`, async () => {
  try {
    const result = await client.request(readItems('productions' as any, {
    ...
    if (process.server && (!result || result.length === 0)) {
...
// Handle 404
if (process.server && !production.value && !error.value) {
...
<template>
  <div class="production-detail">
    <template v-if="production">
      <!-- ═══ Hero ═══ -->
      <section class="production-detail__hero relative pt-8 pb-16" id="production-hero">
      ...
      <!-- ═══ Showtimes ═══ -->
      <section v-if="performances.length" class="production-showtimes production-section py-16" id="showtimes">
      ...
      <!-- ═══ Overview / Content ═══ -->
      <section v-if="production.content" class="production-content py-16 production-section" id="overview">
